...
-- Сборка UI
subtitle.Text = (MODE == "dungeon") and "Dungeon Auto" or "Auto Progression"
makeStatus()
if MODE == "dungeon" then
	section("БОЙ")
	makeToggle("AOE-урон по всем мобам", "dgnAOE", "Залп по всей волне сразу")
	makeRate("dgnRate", "AOE проходов/сек", "суммарно ≤ 400", 1, 30, 1)
	section("ПРОКАЧКА")
	makeToggle("Авто-выбор карт", "dgnCards", "Приоритет: посох > сила > мультицель")
	section("РИСК / НАГРАДЫ")
	makeToggle("Голосовать за модификаторы", "dgnMods", "Усиливают мобов ради монет")
	section("СМЕРТЬ")
	makeToggle("Выход в лобби при смерти", "dgnReturn", "Без робукс-реврана")
else
	section("ПРОГРЕССИЯ")
