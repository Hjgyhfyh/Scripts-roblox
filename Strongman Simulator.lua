-- @sigmatik-protected payload
-- This file is delivered ONLY by the API. Direct distribution is a violation.

-- ---- Integrity / runtime self-check (cheap, executor-portable) ----
do
    local g = getgenv and getgenv() or _G
    -- Loader must have set these. If they're missing we're being called outside the loader.
    if type(g.__SIG_TOKEN) ~= "string" or type(g.__SIG_WM) ~= "string" then
        return
    end
    -- Detect known dump-tool fingerprints in the global env (cheap, last line of defence).
    local hits = { "Cobalt", "RemoteSpy", "Decompiler", "DEX Explorer", "saveinstance", "SaveInstance" }
    if rawget(_G, "saveinstance") or rawget(_G, "SaveInstance") then return end
    if hookmetamethod then
        local s = tostring(hookmetamethod)
        for _, kw in ipairs(hits) do if s:find(kw, 1, true) then return end end
    end
end

-- ---- Periodic heartbeat + kill-switch ----
task.spawn(function()
    while true do
        task.wait(60)
        local hb = getgenv().__SIG_HEARTBEAT
        if type(hb) == "function" then
            local ok = false
            pcall(function() ok = hb() end)
            if not ok then
                -- token revoked / expired -> shut everything down
                pcall(function()
                    local gui = (gethui and gethui() or game:GetService("CoreGui")):FindFirstChild("SigmatikClickGui")
                    if gui then gui:Destroy() end
                    local promo = (gethui and gethui() or game:GetService("CoreGui")):FindFirstChild("SigmatikPromoBanner")
                    if promo then promo:Destroy() end
                end)
                return
            end
        end
