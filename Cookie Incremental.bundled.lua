...
local State = {
    ...
    AutoOpenBasic = false,
    OpenBasicInterval = 0.5,
    BasicInventory = {},
    RuneLabels = {},
}

local BASIC_RARITY_ORDER = { "Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythical", "???" }
local BASIC_BOOSTS = {
...
local function refreshRuneLabels()
    local inv = State.BasicInventory or {}
    for _, rarity in ipairs(BASIC_RARITY_ORDER) do
        local lbl = State.RuneLabels[rarity]
        if lbl and lbl.Set then
            local cnt = inv[rarity] or 0
            ...
            pcall(lbl.Set, lbl, table.concat(lines, "\n"))
        end
    end
end
...
local function captureInventory(res)
    if type(res) ~= "table" or type(res.Inventory) ~= "table" then return end
    table.clear(State.BasicInventory)
    for k, v in pairs(res.Inventory) do
        State.BasicInventory[k] = v
    end
    refreshRuneLabels()
end

local function openBasicOnce()
    local ok, res = pcall(function() return RuneRequest:InvokeServer("OpenBatch", "Basic", 1) end)
    if ok then captureInventory(res) end
end

-- pre-fetch inventory at startup (best-effort, won't block if hangs)
task.spawn(function()
    task.wait(2)
    local ok, res = pcall(function() return RuneRequest:InvokeServer("Get", "Basic") end)
    if ok then captureInventory(res) end
end)

local function basicLoop()
    while State.Alive do
        if not State.AutoOpenBasic then
            task.wait(0.2)
        else
            openBasicOnce()
            task.wait(math.max(0.05, State.OpenBasicInterval))
        end
    end
end

task.spawn(basicLoop)

State.Stop = function()
    State.Alive = false
    State.FarmEnabled = false
    State.UpgradeBuyEnabled = false
    State.AutoOpenBasic = false
    disconnectList(State.CharConnections)
...
local _ctrl
_ctrl, State.Window = Library:Create({
            ...
            Sections = {
                {
                    ...
                    Controls = {
                        {
                            Type = "Button",
                            Name = "Open 1 Basic Rune",
                            Callback = function() openBasicOnce() end,
                        },
                        {
                            Type = "Toggle",
                            Name = "Auto Open Basic Rune",
                            CurrentValue = false,
                            Callback = function(v) State.AutoOpenBasic = v end,
                        },
                        {
                            Type = "Slider",
                            ...
                            Increment = 0.05,
                            CurrentValue = 0.5,
                            Callback = function(v) State.OpenBasicInterval = v end,
                        },
                    },
                },
            },
...
do
    local runesTab = State.Window and State.Window.GetTab and State.Window:GetTab("Runes")
    if runesTab and runesTab.AddSection then
        local statsSection = runesTab:AddSection({ Name = "Basic Inventory & Boost" })
        local refreshBtn = statsSection:AddButton({
        ...
        for _, rarity in ipairs(BASIC_RARITY_ORDER) do
            local lbl = statsSection:AddParagraph({ Name = rarity, Content = "Owned: 0" })
            State.RuneLabels[rarity] = lbl
        end
        refreshRuneLabels()
    end
end
