				...
				for _, cat in UPGRADE_ORDER do
					...
					if list and catData then
						...
						for _, key in UPGRADE_KEYS[cat] do
							if not running or not Config.UpgradeSweep then break end
							local up = list[key]
							if up and type(up.cost) == "function" then
								...
								if lvl < mx then
									...
									if price and bal >= price then
										local id = cat .. "/" .. key
										if now >= (upNextTry[id] or 0) then
											if upLastLevel[id] == lvl then
												upNextTry[id] = now + LOCK_BACKOFF
											else
												if sweepGate() then send("UpgradeUpgradeMax", cat, key) end
												upNextTry[id] = now + 0.4
											end
											upLastLevel[id] = lvl
										end
									end
						...
						if type(node) == "table" then
							...
							if lvl < mx then
								...
								if unlocked then
									...
									if bal >= price then
										if now >= (nextTry[name] or 0) then
											if lastLevel[name] == lvl and lvl > 0 then
												nextTry[name] = now + LOCK_BACKOFF
											else
												if gate() then send(action, name) end
												nextTry[name] = now + 0.3
											end
											lastLevel[name] = lvl
										end
									end
...
makeToggle("Прокачка нубиков", "NoobUpgrade", "Главный доход Oof")
makeToggle("Апгрейды", "UpgradeSweep", "Покупка как только хватает валюты")
makeToggle("Prism дерево", "UITree", "Узлы за Prism")
