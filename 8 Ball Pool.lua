...
-- ============================================================================
-- MODULE: Cue Library — equip any cue from a searchable grid
--   Open: RightShift, or the floating button bottom-right. Hidden by default.
-- ============================================================================
local cueLibTeardown
do
	local ok, err = pcall(function()
		local __clRsConn, __clInputConn
		...
		__clInputConn = UserInputService.InputBegan:Connect(function(input, gpe)
		    if gpe then return end
		    if input.KeyCode == TOGGLE_KEY then setOpen(not backdrop.Visible) end
		end)

		------------------------------------------------------------------- Slow rotation on viewport cards (eye-candy)
		__clRsConn = RunService.RenderStepped:Connect(function(dt)
		...
		cueLibTeardown = function()
			if __clRsConn then __clRsConn:Disconnect() end
			if __clInputConn then __clInputConn:Disconnect() end
			if screen then pcall(function() screen:Destroy() end) end
		end
	end)
	if not ok then warn("[8BallPool] Cue Library disabled: " .. tostring(err)) end
end


-- ============================================================================
-- MODULE: Smooth Opponent Cue — interpolate the opponent's cue between sparse
--   server updates and re-broadcast ours, so both cues move at ~60 Hz.
-- ============================================================================
local smoothCueTeardown
do
	local ok, err = pcall(function()
		local __scGscConn
		...
		if gsc then
		    __scGscConn = gsc.OnClientEvent:Connect(function(_, _, gameData)
		...
		attach()
		smoothCueTeardown = function()
			pcall(function() if _G.SmoothCue then _G.SmoothCue.detach() end end)
			if __scGscConn then __scGscConn:Disconnect() end
			_G.SmoothCue = nil
		end
	end)
	if not ok then warn("[8BallPool] Smooth Opponent Cue disabled: " .. tostring(err)) end
end


-- ============================================================================
-- Anti-AFK (always on) + master unload for the whole suite
-- ============================================================================
local __antiAfk = {}
do
	local ok = pcall(function()
		...
		__antiAfk[#__antiAfk+1] = plr.Idled:Connect(function()
			pcall(function()
	...
	if not ok then warn("[8BallPool] Anti-AFK install failed.") end
end

_G.__EIGHTBALL_SUITE = {
	unload = function()
		pcall(function() if _G.__POOL_PREDICTOR then _G.__POOL_PREDICTOR.cleanup() end end)
		pcall(function() if cueLibTeardown then cueLibTeardown() end end)
		pcall(function() if smoothCueTeardown then smoothCueTeardown() end end)
		for _, c in ipairs(__antiAfk) do pcall(function() c:Disconnect() end) end
		_G.__EIGHTBALL_SUITE = nil
		print("[8BallPool] Suite unloaded.")
	end,
}

print("[8BallPool] Suite ready — Predictor + Cue Library (RightShift) + Smooth Cue + Anti-AFK.  Unload: _G.__EIGHTBALL_SUITE.unload()")
